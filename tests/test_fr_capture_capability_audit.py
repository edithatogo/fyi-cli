...
AUDIT = ROOT / "artifacts" / "foio" / "fr_capture_capability_audit.json"
SCHEMA = ROOT / "schemas" / "fr-capture-capability-audit.schema.json"


def test_fr_capability_audit_is_schema_valid_and_non_executing() -> None:
    audit = json.loads(AUDIT.read_text(encoding="utf-8"))
    schema = json.loads(SCHEMA.read_text(encoding="utf-8"))

    Draft202012Validator(schema).validate(audit)
    assert audit["live_capture_performed"] is False
    assert {item["id"] for item in audit["instances"]} == {"fr-cada"}
    assert all(
...
def test_fr_audit_is_grounded_in_registry_and_capture_implementation() -> None:
