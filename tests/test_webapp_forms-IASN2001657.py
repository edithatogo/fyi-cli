...
def test_insert_and_update_request_helpers(tmp_path: Path):
    ...
    rows = query_all(db, 'SELECT * FROM tracked_requests WHERE id=?', (request_id,))
    assert len(rows) > 0, 'Request should exist after update'
    row = rows[0]
...
def test_render_request_form_contains_prefilled_link(tmp_path: Path):
    ...
    request_id = insert_tracked_request(db, 'alpha', 'Title', 'Body', tags='tag:a', status='draft')
    rows = query_all(db, 'SELECT * FROM tracked_requests WHERE id=?', (request_id,))
    assert len(rows) > 0, 'Request should exist'
    row = rows[0]
    html = _render_request_form(str(db), row)
    assert 'Open FYI prefilled draft' in html or 'prefilled' in html.lower()
