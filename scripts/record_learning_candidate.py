...
def build_entry_lines(message: str, evidence: list[str]) -> list[str]:
    entry = [f"- [ ] {message}"]
    entry.extend(f"  - {item.strip()}" for item in evidence if item.strip())
    return entry
...
def main() -> None:
    ...
    if inserted:
        sys.stdout.write(f"Recorded learning candidate in {backlog}\n")
    else:
        sys.stdout.write(f"Learning candidate already present in {backlog}\n")
