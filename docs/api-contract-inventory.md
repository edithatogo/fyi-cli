...
| rust_sync_client: Rust sync API client | GET /api/v2/request.json, GET /api/v2/request/{id}.json, POST /api/v2/request, watched feed pulls, retry queues, and merge/conflict preservation. | Mocked successful pull, feed pull, malformed JSON, missing required fields, HTTP 401/403/404/429/5xx failures, health, push retry, scheduler, and conflict merge behavior. Evidence: `crates/fyi-core/src/sync.rs`; tests: `crates/fyi-core/src/sync.rs::tests`. | medium |
