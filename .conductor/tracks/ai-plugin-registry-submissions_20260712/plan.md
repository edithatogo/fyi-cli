...
- [~] Task: Reconcile README, registry matrix, manifests, release assets, and current public listings.
    - [x] Add separate Codex and Anthropic-specific child submission packets and review checklists.
    - [x] Record authentication, write-action, privacy, support, and rollback disclosures.
...
- [~] Task: Add deterministic packaging and metadata checks for every target.
    ...
    - [x] Add opt-in authenticated HTTP JSON-RPC transport while preserving stdio defaults. (0910343)
    - [x] Harden HTTP transport protocol negotiation, media-type validation, and no-SSE method behavior.
