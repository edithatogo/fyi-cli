...
async fn test_tor_initialization_and_unbootstrapped_status() {
    ...
    assert!(!status.ready, "Client should not be ready for traffic immediately");
...
async fn test_start_proxy_and_reqwest_client_setup() {
    ...
    assert!(reqwest_client.is_ok(), "Should successfully construct reqwest client configured with proxy");
}
...
async fn test_socks_proxy_handshake_invalid_version() {
    ...
    let mut socket = TcpStream::connect(addr).await.expect("Failed to connect to proxy port");
    ...
    assert_eq!(read_bytes, 0, "Proxy should disconnect on unsupported version");
}
